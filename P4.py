...
# ---
# 
# * Estudiante: **Jasiel Carranza Acuña**
# * Carné: **B71640**
# * Grupo: **1**
# 
# ---
...
def modulador(bits, fc, mpp):
    ...
    # 3. Inicializar la señal modulada s(t)
    t_simulacion = np.linspace(0, N*Tc, N*mpp) 
    senal_Tx = np.empty(t_simulacion.shape)
    moduladora = np.empty(t_simulacion.shape)  # señal de información
     
        
    # 4. Asignar las formas de onda según los bits (QPSK)
    contador=1
    
    #ahora recorro las señales y voy analizando 1 o 0 
    
    for i, bit in enumerate(bits):
        
        #if para la portadora_2
        if (bit == 1) and (contador%2 == 0):
            
            contador = contador + 1
            
            senal_Tx[i*mpp : (i+1)*mpp] = portadora_2
        ...
        if (bit == 1) and (contador%2 != 0):
            ...
            senal_Tx[i*mpp : (i+1)*mpp] = portadora_1
    ...
    Suma_portadoras = portadora_1 + portadora_2
    
    return senal_Tx, Pm, Suma_portadoras, moduladora  
...
def demodulador(senal_Rx, Portadora_Final, mpp):
    ...
    # Energía de un período de la portadora
    Es = np.sum(Suma_portadoras**2)

    # Demodulación
    for i in range(N):
        # Producto interno de dos funciones
        producto = senal_Rx[i*mpp : (i+1)*mpp] * Suma_portadoras
        senal_demodulada[i*mpp : (i+1)*mpp] = producto
        Ep = np.sum(producto) 
...
#  Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, Suma_portadoras, moduladora = modulador(bits_Tx, fc, mpp)

# Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

#  Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, Suma_portadoras, mpp)
...
# Creo el arrreglo con los valores de A
A=[1,-1]

# Defino el tiempo de muestreo
tiempo = np.linspace(0, 0.1,100)

# creo la onda
X_t = np.empty((4, len(t_muestra)))	
plt.figure()

# bucle para llenar la matriz  
for i in A:
    xa = i * np.cos(2*(np.pi)*fc*t_muestra) +  i *np.sin(2*(np.pi)*fc*t_muestra)
    xb = -i * np.cos(2*(np.pi)*fc*t_muestra) +  i *np.sin(2*(np.pi)*fc*t_muestra) 
    X_t[i,:] = xa
    X_t[i+1,:] = xb
    plt.plot(tiempo,xa, lw=2)
    plt.plot(tiempo, xb, lw=2)       

#Saco el promedio 
Promedio = [np.mean(X_t[:,i]) for i in range(len(t_muestra))]
plt.plot(tiempo, Promedio, lw=6,color='c',label='Promedio de las realizaciones')

# grafica comparativa, valor teorico y las realizaciones
teorico = np.mean(senal_Tx)*t_muestra  
plt.plot(t_muestra, teorico, '-.', lw=3,color='g',label='Resultado teórico')
