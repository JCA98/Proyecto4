...
for i in A:
    xa = i * np.cos(2*(np.pi)*fc*tiempo) +  i *np.sin(2*(np.pi)*fc*tiempo)
    xb = -i * np.cos(2*(np.pi)*fc*tiempo) +  i *np.sin(2*(np.pi)*fc*tiempo) 
    X_t[i,:] = xa
    X_t[i+1,:] = xb
...
#Saco el promedio 
Promedio = [np.mean(X_t[:,i]) for i in range(len(tiempo))]
...
teorico = np.mean(senal_Tx)*tiempo 
plt.plot(tiempo, teorico, '-.', lw=3,color='g',label='Resultado teórico')
